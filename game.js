...
function stopSE(){Object.values(sePool).flat().forEach(a=>{a.pause();a.currentTime=0;});}
function se(kind,c,strength=.4){
  if(!soundOn||hidden)return;
  if(kind==='win'){
    stopSE();const a=sePool.yahhoo[0];a.volume=.85;a.play().catch(()=>{});return;
  }
  if(mode==='escaping'||mode==='result')return;
  const now=performance.now();if(now-seLastWater<140)return;
...
function setup(){const heights=[.42,.55,.46,.59,.40],space=Math.min(82,W*.175);chars=palette.map((color,id)=>{const home=W/2+(id-2)*space,hp=TEAM_HP;return {id,color,home,anchor:home,homeY:H*heights[id],x:home,y:H*heights[id],vx:0,vy:0,r:Math.min(28,W*.066),hp,maxHp:hp,charge:0,hold:0,twist:0,reach:.12,yaw:id*.4,spin:0,tilt:0,stretch:0,pop:0,recoil:0,cool:0,active:0,lastDir:id%2?1:-1,free:false,escape:0,trail:0};});links=[];for(let a=0;a<5;a++)for(let b=a+1;b<5;b++)links.push({a,b,power:0,cool:0,bodyCool:0,tA:.6,tB:.6,x:0,y:0,phase:0,turns:1.4,span:.17,spinDir:1});particles=[];words=[];pointer=null;teamHp=TEAM_HP;damagePulse=0;escapeTime=0;lastDrag=lastInput=-10;endWin=false;elapsed=0;caught=0;combo=0;comboTime=0;shake=flash=0;stats={taps:0,drags:0,flicks:0,recoils:0,tangles:0,bonks:0};}
function start(){unlockAudio();stopSE();seIndex=0;seLastWater=-Infinity;setup();mode='play';$('#start-panel').classList.add('hidden');$('#result-panel').classList.add('hidden');$('#hud').classList.remove('hidden');oldBgm.currentTime=0;nextBeat=audio?.currentTime||0;beat=0;syncMusic();label('つつく！ のばす！ はなす！',W/2,H*.76,'#ffecaa',2);}
function label(){}
...
function releaseAll(){
  if(mode!=='play')return;
  mode='escaping';endWin=true;escapeTime=0;pointer=null;teamHp=0;
  chars.forEach(c=>{
...
function interact(dt){const paths=chars.map(c=>c.free?null:ropePath(c));links.forEach(k=>{k.cool=Math.max(0,k.cool-dt);k.bodyCool=Math.max(0,k.bodyCool-dt);k.power=Math.max(0,k.power-dt*.22);k.phase+=dt*(3+k.power*5)*k.spinDir;const a=chars[k.a],b=chars[k.b];if(a.free||b.free||mode!=='play')return;const relative=Math.hypot(a.vx-b.vx,a.vy-b.vy);if((a.active>0||b.active>0)&&relative>35&&k.cool===0){let best=23,hit=null;for(let i=10;i<65;i+=3)for(let j=10;j<65;j+=3){const p=paths[k.a][i],q=paths[k.b][j],d=Math.hypot(p.x-q.x,p.y-q.y);if(d<best){best=d;hit={p,q};}}if(hit){k.tA=clamp(hit.p.t,.34,.76);k.tB=clamp(hit.q.t,.34,.76);const pa=paths[k.a][Math.round(k.tA*64)],pb=paths[k.b][Math.round(k.tB*64)];k.x=(pa.x+pb.x)/2;k.y=(pa.y+pb.y)/2;k.power=1;k.cool=.9;stats.tangles++;combo=comboTime>0?combo+1:1;comboTime=2.1;const stored=clamp((Math.abs(a.twist)+Math.abs(b.twist))/ (100/12),0,2),dir=a.x>b.x?1:-1;k.spinDir=dir;k.turns=clamp(.65+stored*.22+relative/1600,.65,1.35);k.span=clamp(.17+stored*.025,.17,.21);a.vx+=dir*(72+stored*35);b.vx-=dir*(72+stored*35);a.vy-=55;b.vy+=40;a.spin=dir*20;b.spin=-dir*20;const hitPower=contactPower(a,b);a.charge*=.96;b.charge*=.96;a.active=b.active=1;damageTeam(hitPower);if(mode!=='play')return;burst(k.x,k.y,Math.round(5+stored*9),'#d9fff4');label(stored>1?'こすれて効いた！':combo>1?`こすれ ${combo}連鎖！`:'キュッ！',k.x,k.y-18,'#e0fff4');se('tangle',a,stored/2);shake=1+stored*2;}}
...
function mermaid(front=false){
 ...
 if((mode!=='play'&&!engulfed)||elapsed<3||front!==engulfed)return;
 const p=clamp((elapsed-3)/(ROUND_SECONDS-3),0,1),lunge=engulfed?clamp(caught/1.2,0,1):0;
...
function finish(win){if(mode!=='play'&&mode!=='caught'&&mode!=='escaping')return;const bitten=mode==='caught';mode='result';endWin=win;pointer=null;oldBgm.pause();if(!win&&!bitten)se('lose');$('#result-panel').classList.remove('hidden');$('#hud').classList.add('hidden');$('#result-panel .eyebrow').textContent=win?'ぷちんっ、すいすい。全員脱出！':'人魚につかまった！';
$('#result-score').textContent=win?elapsed.toFixed(1):'あと少し';$('#result-unit').textContent=win?'秒で、自由！':'';$('#result-copy').textContent=win?(elapsed<30?'ねじりの達人！ 海の向こうまで、すいすい。':'みんな無事！ いっぱい暴れて、いっぱいねじれた。'):'みんなをねじって、昆布同士をぶつけよう。';$('#result-stats').textContent=`くるるん ${stats.recoils}回　·　からまり ${stats.tangles}回`;
try{const key='konbu-team30-best-v1',best=Number(localStorage.getItem(key))||Infinity;if(win&&elapsed<best)localStorage.setItem(key,String(elapsed));const value=Math.min(win?elapsed:Infinity,best);$('#best-score').textContent=Number.isFinite(value)?`この端末のベスト ${value.toFixed(1)}秒`:'';}catch{$('#best-score').textContent='';}}
...
document.addEventListener('visibilitychange',()=>{hidden=document.hidden;pointer=null;last=performance.now();if(hidden){oldBgm.pause();stopSE();audio?.suspend();}else{if(soundOn)audio?.resume().catch(()=>{});syncMusic();}});
