...
canvas.addEventListener('pointerdown',e=>{inputDebug.downs++;if(mode!=='play')return;if(e.cancelable)e.preventDefault();unlockAudio();pointers=pointers.filter(q=>q.id!==e.pointerId);const c=pick(e.clientX,e.clientY);if(!c){inputDebug.misses++;inputDebug.last='miss';return;}const p={id:e.pointerId,c,sx:e.clientX,sy:e.clientY,x:e.clientX,y:e.clientY,lx:e.clientX,ly:e.clientY,moved:0,bank:0,time:clock,mx:0,my:0,mt:clock,offsetX:c.x-e.clientX,offsetY:c.y-e.clientY};pointers.push(p);if(!pointer)pointer=p;if(pointers.length>=2){lastDrag=lastInput=clock;pointers.forEach(q=>{q.c.active=1.8;pushMotion(q.c,q.c.lastDir*14,-9);});if(clock-multiTapAt>.18){multiTapAt=clock;damageTeam(2.1);burst(c.x,c.y,6,'#e5ffd0');}}try{canvas.setPointerCapture(e.pointerId);}catch{}c.pop=.3;});
