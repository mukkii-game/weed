...
function ropePath(c){
  ...
  for(let i=0;i<=64;i++){
    ...
    links.forEach(k=>{
      if(k.power<.02||(k.a!==c.id&&k.b!==c.id)||chars[k.a].free||chars[k.b].free)return;
      const kt=k.a===c.id?k.tA:k.tB,side=(k.a===c.id?1:-1)*k.spinDir,s=(t-kt)/k.span;
      if(Math.abs(s)>=1)return;
      const envelope=Math.cos(s*Math.PI/2)**2*k.power;
...
function ribbon(c,pts=ribbonGeometry(c),front=false){
  ...
  for(const side of ['l','r']){
    ...
    ctx.strokeStyle='#204f3c';ctx.lineWidth=1.25;ctx.stroke();
  }
