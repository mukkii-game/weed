...
function makeTint(color,id){
  ...
  const x=c.getContext('2d');x.drawImage(sprite,0,0);x.globalCompositeOperation='source-atop';
  x.globalAlpha=.34;x.fillStyle=color;
  if(id===1){x.fillRect(0,0,c.width,c.height*.48);}
  else if(id===3){x.fillRect(0,c.height*.53,c.width,c.height*.47);}
  else{x.fillRect(0,0,c.width,c.height);}
  return c;
}
sprite.onload=()=>palette.forEach((color,id)=>tints.push(makeTint(color,id)));
const SPRITE_FRAMES=7;
// The diagonal drawings have the same pixel-height bbox, but their round silhouette
// reads smaller after yaw/tilt. Give those four views a small optical compensation.
const SPRITE_VIEW_SCALE=[1,1.08,1,1.08,1,1.08,1.08];
// The profile frame is 223px wide vs 265px for the front frame. Compensate
// only its horizontal scale so the intentionally round character stays round.
const SPRITE_VIEW_X_SCALE=[1,1,1.28,1,1,1,1];
// Front-left and front-right drawings keep their top aligned but need a little
// downward optical correction so their feet sit on the same baseline.
const SPRITE_VIEW_Y_OFFSET=[0,.065,0,.065,0,0,0];
// Visible (alpha >= 128) bounds, not faint antialiasing pixels. The two
// front-diagonal drawings are much shorter in the source art, so stretch them
// vertically while keeping their visible top and bottom on the same baselines.
const SPRITE_BBOX=[[17,37,282,270],[10,39,280,230],[38,37,261,270],[21,40,274,221],[16,37,284,270],[3,40,279,244],[5,40,293,258]];
const SPRITE_VIEW_Y_SCALE=[1,233/(191*1.08),1,233/(181*1.08),1,233/(204*1.08),233/(218*1.08)];
function spriteFootShift(view,size,sx,sy,rot){
  const b=SPRITE_BBOX[view],ref=SPRITE_BBOX[0],xs=[(b[0]-150)*size/300,(b[2]-150)*size/300],ys=[(b[1]-150)*size/300*SPRITE_VIEW_Y_SCALE[view],(b[3]-150)*size/300*SPRITE_VIEW_Y_SCALE[view]];
  let top=Infinity;for(const x of xs)for(const y of ys)top=Math.min(top,Math.sin(rot)*sx*x+Math.cos(rot)*sy*y);
  const refTop=(ref[1]-150)*size/300*sy;
  return refTop-top+size*SPRITE_VIEW_Y_OFFSET[view];
}
...
let elapsed=0,clock=0,last=0,lastInput=-10,inputDebug={downs:0,ups:0,cancels:0,misses:0,last:''};try{globalThis.konbuInputDebug=inputDebug;}catch{};
let caught=0,dissipateTime=0,shake=0,flash=0,combo=0,comboTime=0,multiTapAt=-Infinity,stats={},endWin=false;
let soundOn=true,musicMode=1,audio=null,beat=0,nextBeat=0,hidden=false;
const oldBgm=new Audio('assets/maou-orchestra24.mp3?v=1');oldBgm.loop=true;oldBgm.volume=.14;
...
const biteSE=Array.from({length:3},()=>{const a=new Audio('assets/se/biting.mp3');a.preload='auto';return a;});
let cheerTimers=[],biteTimers=[],startCheerTimer=null,lastPowerSound=-Infinity;
...
function playClip(pool,volume=.7,rate=1){const a=pool.find(q=>q.paused||q.ended)||pool[0];if(!a)return;a.pause();a.currentTime=0;a.volume=volume;a.playbackRate=rate;a.play().catch(()=>{});return a;}
function stopSE(){cheerTimers.forEach(id=>{if(typeof clearTimeout==='function')clearTimeout(id);});biteTimers.forEach(id=>{if(typeof clearTimeout==='function')clearTimeout(id);});cheerTimers=[];biteTimers=[];if(startCheerTimer!==null&&typeof clearTimeout==='function')clearTimeout(startCheerTimer);startCheerTimer=null;[...cheerVoices,...Object.values(sePool).flat(),...tighteningSE,...knifeSE,...biteSE,startMaleSE,startCheerSE].filter(a=>typeof a.pause==='function').forEach(a=>{a.pause();a.currentTime=0;});}
function se(kind,c,strength=.4){
  ...
  if(kind==='escape'){
    const now=performance.now(),file=waterFiles[(seIndex+1)%waterFiles.length],a=sePool[file].find(q=>q.paused||q.ended);
    if(a){seIndex++;seLastWater=now;a.currentTime=0;a.volume=.62;a.playbackRate=.78;a.play().catch(()=>{});}
    tone(118,.28,.12,'sine',0,42);tone(360,.22,.045,'triangle',.025,150);
    return;
  }
  if(kind==='caught'){
    stopSE();
    const playBite=i=>{const a=biteSE[i];a.currentTime=0;a.volume=.78;a.playbackRate=.5;a.play().catch(()=>{});};
    biteSE.forEach((_,i)=>{const fire=()=>playBite(i);biteTimers.push(typeof setTimeout==='function'?setTimeout(fire,i*520):(fire(),0));});
    return;
  }
  if(kind==='start'){
    playClip([startMaleSE],.82,.98);
    const fire=()=>playClip([startCheerSE],.74,1);
    if(typeof setTimeout==='function')startCheerTimer=setTimeout(fire,500);else fire();
    return;
  }
  if(mode==='escaping'||mode==='result')return;
  const now=performance.now();
  if(kind==='recoil'){
    playClip(tighteningSE,.78,.94);
    return;
  }
  if(kind==='power'){
    if(now-lastPowerSound<280)return;
    lastPowerSound=now;
    playClip([knifeSE[Math.random()<.5?0:1]],.86,.92);
...
function contactFeedback(a,b,k,hitPower){
  ...
  if(charged<.82||hitPower<9||clock-lastDrag>.65)return false;
  ...
  a.hitFlash=b.hitFlash=.25;k.flash=.25;se('power',a,strength);shake=Math.max(shake,1.5+strength*2);return true;
}
...
function recoil(c){c.charge=100;c.hold=3;c.cool=3;c.recoil=.25;c.edgeFlash=.24;c.spin*=.22;c.pop=.24;stats.recoils++;shake=2;se('recoil',c,.75);burst(c.x,c.y-c.r,8);}
function releaseAll(){
  if(mode!=='play')return;
  mode='dissipating';endWin=true;escapeTime=0;dissipateTime=0;pointer=null;pointers=[];teamHp=0;
  chars.forEach(c=>{
    const angle=-Math.PI/2+(c.id-2)*.27,speed=Math.max(W,H)*.31;
    c.free=true;c.hp=0;c.escape=0;c.escapeVx=Math.cos(angle)*speed;c.escapeVy=Math.sin(angle)*speed;c.vx=c.escapeVx;c.vy=c.escapeVy;c.spin=0;c.yaw=1.2;c.tilt=-.2;c.eat=0;
    burst(c.x,c.y-c.r,18,'#fff2a6');
    for(let i=0;i<18;i++){
      const a=angle+(Math.random()-.5)*.8,v=75+Math.random()*170;
      particles.push({kind:'shard',x:c.x+(Math.random()-.5)*c.r*1.4,y:c.y+(Math.random()-.5)*c.r*1.4,vx:Math.cos(a)*v,vy:Math.sin(a)*v,r:1.5+Math.random()*3.5,w:4+Math.random()*7,h:12+Math.random()*18,angle:Math.random()*TAU,spin:(Math.random()-.5)*18,life:.85+Math.random()*.65,color:i%3?'#a9df9a':'#f2ffc4'});
    }
  });
  links.forEach(k=>{k.power=0;});stats.rescued=chars.length;
  shake=8;flash=.18;oldBgm.pause();se('win');se('escape');
  if(navigator.vibrate)navigator.vibrate([30,40,50]);
}
function pick(x,y){let best=null,dist=72;chars.forEach(c=>{const d=Math.hypot(c.x-x,c.y-y),limit=Math.max(dist,c.r*2.25);if(!c.free&&d<limit){dist=d;best=c;}});return best;}
...
canvas.addEventListener('pointerdown',e=>{inputDebug.downs++;if(mode!=='play')return;if(e.cancelable)e.preventDefault();unlockAudio();const isTouch=e.pointerType==='touch';pointers=pointers.filter(q=>q.id!==e.pointerId&&clock-(q.lastEvent??q.time)<1.5);if(pointer&&!pointers.includes(pointer))pointer=null;const c=pick(e.clientX,e.clientY);if(!c){inputDebug.misses++;inputDebug.last='miss';return;}const p={id:e.pointerId,c,sx:e.clientX,sy:e.clientY,x:e.clientX,y:e.clientY,lx:e.clientX,ly:e.clientY,moved:0,bank:0,time:clock,lastEvent:clock,mx:0,my:0,mt:clock,offsetX:c.x-e.clientX,offsetY:c.y-e.clientY};pointers.push(p);if(!pointer)pointer=p;if(pointers.length>=2){lastDrag=lastInput=clock;pointers.forEach(q=>{q.c.active=1.8;pushMotion(q.c,q.c.lastDir*14,-9);});if(clock-multiTapAt>.18){multiTapAt=clock;damageTeam(2.1);burst(c.x,c.y,6,'#e5ffd0');}}if(!isTouch)try{canvas.setPointerCapture(e.pointerId);}catch{}c.pop=.3;});
canvas.addEventListener('pointermove',e=>{if(e.cancelable&&mode==='play')e.preventDefault();const p=pointers.find(q=>q.id===e.pointerId);if(!p)return;p.lastEvent=clock;const x=clamp(e.clientX,10,W-10),y=clamp(e.clientY,H*.18,H*.8),dx=x-p.x,dy=y-p.y,dt=Math.max(.016,clock-p.mt);p.mx=clamp(dx/dt,-1100,1100);p.my=clamp(dy/dt,-1100,1100);p.mt=clock;p.x=x;p.y=y;p.moved+=Math.hypot(dx,dy);p.bank+=Math.hypot(dx,dy);if(p.moved>12&&!p.c.free){lastDrag=lastInput=clock;p.c.active=1.5;if(p.bank>16){const energy=Math.min(12,p.bank*.17);p.bank=0;energize(p.c,energy,Math.sign(dx)||p.c.lastDir,'drag');stats.drags++;if(Math.random()<.3)burst(p.c.x,p.c.y,2);if(clock-(p.soundAt||0)>.16){se('stretch',p.c);p.soundAt=clock;}}}});
function pointerEnd(e,cancel=false){cancel?inputDebug.cancels++:inputDebug.ups++;const p=pointers.find(q=>q.id===e.pointerId);if(!p)return;pointers=pointers.filter(q=>q!==p);if(pointer===p)pointer=pointers[0]||null;if(e.pointerType!=='touch'&&canvas.hasPointerCapture?.(e.pointerId))canvas.releasePointerCapture(e.pointerId);if(cancel||p.c.free||mode!=='play')return;const c=p.c;if(p.moved<14&&Math.hypot(e.clientX-p.sx,e.clientY-p.sy)<18){stats.taps++;c.vy-=90;c.vx+=c.lastDir*32;energize(c,24,c.lastDir,'tap');se('tap',c);burst(c.x,c.y+c.r,5);label('くるるっ',c.x,c.y-48,c.color,.6);}else{lastDrag=lastInput=clock;stats.flicks++;const dx=p.x-p.sx,dy=p.y-p.sy,len=Math.hypot(dx,dy),fresh=clock-p.mt<.15;const edge=c.id===0||c.id===4?1.25:1;c.vx=clamp((fresh?p.mx*.38:0)+(c.home-c.x)*2,-540,540)*edge;c.vy=clamp((fresh?p.my*.38:0)+(c.homeY-c.y)*2,-520,520);energize(c,clamp(len*.13,10,32),Math.sign(dx)||c.lastDir,'flick');c.stretch=.35;burst(c.x,c.y,9);se('tap',c);label(len>W*.25?'びよーーん！':'びゅんっ！',c.x,c.y-48);}}
...
addEventListener('pointerup',e=>pointerEnd(e));addEventListener('pointercancel',e=>pointerEnd(e,true));addEventListener('blur',()=>{pointer=null;pointers=[];inputDebug.last='blur-reset';});
...
function beginCaught(){if(mode!=='play')return;mode='caught';caught=0;chars.forEach(c=>c.eat=0);pointer=null;pointers=[];oldBgm.pause();se('caught');shake=5;label('みつかった……！',W*.58,H*.42,'#ffe6cb',1.2);}
function update(dt){clock+=dt;if(pointers.length){const before=pointers.length;pointers=pointers.filter(p=>clock-(p.lastEvent??p.time)<2);if(before!==pointers.length){if(pointer&&!pointers.includes(pointer))pointer=pointers[0]||null;inputDebug.last='stale-reset';}}damagePulse=Math.max(0,damagePulse-dt);impacts.forEach(e=>e.age+=dt);impacts=impacts.filter(e=>e.age<.65);shake*=Math.exp(-dt*12);flash=Math.max(0,flash-dt);comboTime-=dt;particles.forEach(p=>{p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vx*=Math.exp(-dt*2);p.vy-=45*dt;if(p.kind==='shard')p.angle+=p.spin*dt;});particles=particles.filter(p=>p.life>0);words.forEach(w=>{w.life-=dt;w.y-=dt*17;});words=words.filter(w=>w.life>0);
...
chars.forEach(c=>{if(c.free){if(mode!=='escaping')return;c.escape+=dt;c.vx=mix(c.vx,c.escapeVx||0,1-Math.exp(-dt*2.4));c.vy=mix(c.vy,c.escapeVy||0,1-Math.exp(-dt*2.4));c.x+=c.vx*dt;c.y+=c.vy*dt;c.yaw=1.2+Math.sin(c.escape*12+c.id)*.13;c.tilt=-.2+Math.sin(c.escape*12+c.id)*.32;c.spin=18*Math.sin(c.escape*12+c.id);c.trail-=dt;if(c.trail<0){burst(c.x,c.y+c.r,2,'#d8f7b0');c.trail=.1;}return;}if(mode!=='play')return;c.active=Math.max(0,c.active-dt);c.cool=Math.max(0,c.cool-dt);c.recoil=Math.max(0,c.recoil-dt);c.pop*=Math.exp(-dt*9);c.stretch*=Math.exp(-dt*7);const held=pointer?.c===c&&pointer.moved>12;
...
}else if(mode==='dissipating'){dissipateTime+=dt;if(dissipateTime>=DISSIPATE_SECONDS){mode='escaping';escapeTime=0;}}else if(mode==='escaping'){escapeTime+=dt;if(escapeTime>=ESCAPE_SECONDS)finish(true);}
...
function character(c){if(mode==='caught'&&c.eat>=1.15)return;const cos=Math.cos(c.yaw),sin=Math.sin(c.yaw),view=cos>.78?0:cos>.2?(sin>=0?3:1):cos>-.55?2:cos>-.9?(sin>=0?6:5):4,size=c.r*3.05*SPRITE_VIEW_SCALE[view],squash=c.pop+Math.min(.22,Math.abs(c.vy)*.00035),fade=c.free&&mode==='escaping'?clamp(1-c.escape/ESCAPE_SECONDS,0,1):1,eat=mode==='caught'?clamp(c.eat,0,1):0;ctx.save();ctx.globalAlpha=fade*(1-eat*.35);ctx.translate(c.x,c.y+spriteFootShift(view,size,(.86+.14*Math.abs(cos))*SPRITE_VIEW_X_SCALE[view]*(1-eat*.88)*(1+squash*.4),(1-squash*.25)*(1-eat*.88),c.tilt+Math.sin(c.yaw)*Math.min(.25,Math.abs(c.spin)*.009)));ctx.rotate(c.tilt+Math.sin(c.yaw)*Math.min(.25,Math.abs(c.spin)*.009));ctx.scale((.86+.14*Math.abs(cos))*SPRITE_VIEW_X_SCALE[view]*(1-eat*.88)*(1+squash*.4),(1-squash*.25)*(1-eat*.88)*SPRITE_VIEW_Y_SCALE[view]);if(view===2&&sin>0)ctx.scale(-1,1);if(sprite.complete&&sprite.naturalWidth){const sw=sprite.width/SPRITE_FRAMES;ctx.drawImage(tints[c.id]||sprite,view*sw,0,sw,sprite.height,-size/2,-size/2,size,size);}else{ctx.fillStyle=c.color;ctx.beginPath();ctx.ellipse(0,0,c.r,c.r*.92,0,0,TAU);ctx.fill();ctx.fillStyle='#233c44';if(cos>0){for(const x of [-8,8]){ctx.beginPath();ctx.arc(x,-2,2.4,0,TAU);ctx.fill();}ctx.beginPath();ctx.arc(0,4,4,0,Math.PI);ctx.stroke();}}ctx.restore();if(Math.abs(c.spin)>9){ctx.strokeStyle=c.recoil>0?'#ffed9f':'#c8fff1';ctx.lineWidth=2;ctx.globalAlpha=.6*fade;for(let i=0;i<2;i++){ctx.beginPath();ctx.ellipse(c.x,c.y,c.r*1.6,c.r*.52,-.2,clock*13+i*Math.PI,clock*13+i*Math.PI+1.9);ctx.stroke();}ctx.globalAlpha=1;}
if(c.hitFlash>0||c.edgeFlash>0){ctx.save();ctx.globalAlpha=.78;ctx.strokeStyle=c.hitFlash>0?'#ffffff':'#ff5b62';ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=18;ctx.lineWidth=5;ctx.beginPath();ctx.arc(c.x,c.y,c.r*1.34,0,TAU);ctx.stroke();ctx.restore();c.hitFlash=Math.max(0,c.hitFlash-.06);c.edgeFlash=Math.max(0,c.edgeFlash-.06);}
...
function mermaid(front=false){
 const threat=mode==='caught'||mode==='dissipating'||mode==='escaping'||(mode==='result'&&!endWin)||(mode==='result'&&endWin);
 ...
 ctx.shadowBlur=0;if(p>.8){ctx.fillStyle='#ffffbb';ctx.beginPath();ctx.ellipse(0,r*.33,r*(.34+lunge*.10),r*(.10+lunge*.19),0,0,TAU);ctx.fill();}ctx.restore();
}

function updateHpUi(){const hp=clamp(teamHp/TEAM_HP,0,1),damage=1-hp,score=$('#score'),fill=$('#damage-fill'),track=$('.damage-track');if(score)score.textContent=`${Math.round(hp*100)}%`;if(fill)fill.style.transform=`scaleX(${damage})`;if(track?.classList){if(hp<=1/3)track.classList.add('danger');else track.classList.remove('danger');}}
function draw(){sea();mermaid(mode==='caught'||mode==='dissipating'||mode==='escaping'||mode==='result');ctx.save();ctx.translate(Math.sin(clock*67)*shake,Math.cos(clock*83)*shake);ribbons();frictionEffects();impactEffects();grabFeedback();chars.forEach(character);particles.forEach(p=>{ctx.globalAlpha=clamp(p.life/DISSIPATE_SECONDS,0,1);if(p.kind==='shard'){ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.angle);ctx.fillStyle=p.color;ctx.fillRect(-p.w/2,-p.h/2,p.w,p.h);ctx.restore();}else{ctx.strokeStyle=p.color;ctx.lineWidth=1.6;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,TAU);ctx.stroke();}});ctx.globalAlpha=1;words.forEach(w=>{ctx.globalAlpha=clamp(w.life*3,0,1);ctx.textAlign='center';ctx.font='900 17px system-ui';ctx.lineWidth=4;ctx.strokeStyle='#164957';const x=clamp(w.x,85,W-85);ctx.strokeText(w.text,x,w.y);ctx.fillStyle=w.color;ctx.fillText(w.text,x,w.y);});ctx.restore();ctx.globalAlpha=1;if(flash>0){ctx.fillStyle=`rgba(226,255,210,${flash})`;ctx.fillRect(0,0,W,H);}if(mode==='play'||mode==='caught'||mode==='dissipating'||mode==='escaping'){updateHpUi();$('#distance').textContent=Math.max(0,Math.ceil(ROUND_SECONDS-elapsed));const timerFill=$('#timer-fill');if(timerFill)timerFill.style.transform=`scaleX(${Math.max(0,1-elapsed/ROUND_SECONDS)})`;}}
function finish(win){if(mode!=='play'&&mode!=='caught'&&mode!=='escaping')return;const bitten=mode==='caught';mode='result';endWin=win;pointer=null;pointers=[];oldBgm.pause();if(!win&&!bitten)se('lose');$('#result-panel').classList.remove('hidden');$('#hud').classList.add('hidden');$('#result-score').textContent=win?elapsed.toFixed(1):'時間切れ';$('#result-unit').textContent=win?'秒':'';try{const key='konbu-team30-best-v1',best=Number(localStorage.getItem(key))||Infinity;if(win&&elapsed<best)localStorage.setItem(key,String(elapsed));const value=Math.min(win?elapsed:Infinity,best);$('#best-score').textContent=Number.isFinite(value)?`この端末の最短\nベストタイム ${value.toFixed(1)}秒`:'';}catch{$('#best-score').textContent='';}}
